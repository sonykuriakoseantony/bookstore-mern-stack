      ...
      <section className='my-5 p-5 md:px-40 flex flex-col justify-center items-center'>
        ...
        <div className='md:grid grid-cols-4 w-full my-10'>
...

          {
            homeBooks?.length > 0 ?
              homeBooks?.map(book => (
                <div key={book?._id} className='shadow rounded-lg p-3 mx-4 my-3 md:mt-0 md:mb-8'>
                  <div style={{ width: '100%', height : '350px', overflow : 'hidden' }}>
                    <img src={book?.imageURL} alt="Alchemist" style={{ width: '100%', height : '100%', objectFit : 'cover' }} />
                  </div>
                 
                  <div className='flex flex-col justify-center items-start m-4'>
                    <h3 className='text-blue-800 font-bold text-xl'>{book?.author}</h3>
                    <p>{book?.title}</p>
